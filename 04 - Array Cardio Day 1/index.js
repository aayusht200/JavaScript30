...
function ageSort() {
  let temp = inventors.map((byAge) => ({
    first: byAge.first,
    last: byAge.last,
    age: byAge.passed - byAge.year,
  }));
  //   console.table(temp);
  let ageSortedInventors = temp.sort((a, b) => a.age - b.age);
...
function yearsLived() {
  console.table(ageSort().sort((a, b) => b.age - a.age));
}
// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
// https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
function boulevardsDeParis() {
  let newBoulevard = boulevardsParis.filter((name) =>
    name.toLowerCase().includes("de")
  );
  console.table(newBoulevard);
}
// 7. sort Exercise
